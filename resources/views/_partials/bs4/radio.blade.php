    ...
    @foreach($data as $item)
        ...
        <div class="radio">
            <label  class="col-form-label" for="{{ $input }}_{{ $item['label'] }}">
                <input type="radio" id="{{ $input }}_{{ $item['label'] }}"    name="{{ $input }}" value="{{ $item['value'] }}" @if(isset($autofocus)) autofocus @endif @if(isset($required)) required="required" @endif   @if($item['checked'])   checked = "checked" @endif >
                {{ $item['label'] }}
            </label>
